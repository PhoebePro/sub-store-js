...
config.outbounds.map(i => {
  ...
  if (['🇨🇳 TW'].includes(i.tag)) {
    i.outbounds.push(...getTags(proxies, /^(?!.*[ACA]).*(台湾|tw|taiwan|🇹🇼).*/i))
  }
  if (['🇯🇵 JP'].includes(i.tag)) {
    i.outbounds.push(...getTags(proxies, /^(?!.*[ACA]).*(日本|jp|japan|🇯🇵).*/i))
  }
  if (['🇸🇬 SG'].includes(i.tag)) { 
    i.outbounds.push(...getTags(proxies, /^(?!.*[ACA]).*(新|sg|singapore|🇸🇬).*/i))
  }
